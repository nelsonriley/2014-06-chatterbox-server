...
module.exports.handler = function (request, response) {
  ...
  if(request.method === "OPTIONS") {
    ...
    ///// Respond to API Requests for Messages and Rooms
...
// load messages from stored json file and build initial response object
var responseObject = {
  results: []
};

var initResponseObject = function(messages) {
  responseObject['results'] = messages;
};

fs.readFile('./savedChats.json', 'utf8', function (err,data) {
  if (err) {
    return console.log(err);
  }
  initResponseObject(JSON.parse(data));
});

// write to stored messages whenever a message is added or removed
var saveMessageJSON = function() {
  var json = JSON.stringify(responseObject.results);
  fs.writeFile('./savedChats.json', json, 'utf8', function(err) {
    if (err) { return console.log(err); }
    console.log("Saved Messages to File");
  });
};

// add a message to the list of results
